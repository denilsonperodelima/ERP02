  		$(document).ready(function(){
			$('#fornecedorcadastro').bind('submit',function(e){
	  			e.preventDefault();
	  		});
  		});
  	    ...
  	    window.onload = function() {  
  	    	  ...
  	    	  operacaoGET("/grupo/1/"  + $("#tiporelacionamento").val(), "carregarcombos")
        ...
        $("#bntEditarDoc").click( function (e) {
    		...
    		var doc = $("#documento").val()
  			var url = getAmbiente() + "/pessoa/documento/1/"  + $("#tiporelacionamento").val() + "/" + $("#documento").val().replace("/","+").replace(".","x").replace(".","x");
  			
  			iniciarFormulario();
  			
  			$("#documento").val(doc);
  			consultarFornecedor(url)     		
        })
        
     		
        $("#gravarfornecedorcadastro").click( function (e) {
     		e.preventDefault();
        ...
        $("#novofornecedorcadastro").click( function (e) {
     		e.preventDefault();
	    	iniciarFormulario();
         });
 ...
 //---------------------------------------------------    	
	  ...
	  $(document).on("click", "#botaopesquisarnome", function(e){	
		...
		var url = getAmbiente() + "/pessoa/nome/1/"  + $("#tiporelacionamento").val() + "/" + $("#nomepesquisa").val()
		consultarPessoaNome(url)
		...
		$(document).on("click", "#selecionarpessoaitem", function(e){	
 			e.preventDefault();
  			...
  			var url = getAmbiente() + "/fornecedor/"  + $("#id").val() 
...
//----------------------------------------------------------------------------------------  
     ...
     */
  	...
  	function iniciarFormulario(){
		...
		$("#cep").mask("00000-000");
		$("#tiporelacionamento").val("FORNECEDOR");
	...
	function limparFormulario(){
			$('#fornecedorcadastro').each (function(){
	   			  this.reset();
	   			});
	  };  	
	...
	function atualizarFornecedor(jsonparam){
	...
	function consultarPessoaNome(url) {
	    $.ajax({
	    	type: 'GET',
	    	url: url ,
	    	dataType:'json',
	...
	function consultarFornecedor(url) {
	    $.ajax({
	    	type: 'GET',
	    	url: url ,
	    	dataType:'json',
  		      	...
  		      	case "carregarcombos":  		      			  		      		
		  		  	    $.each(response.subgrupos, function(i, item) {	
		  	  		       	...
		  	  		       	switch (item.descricao) { 	
		  	  		         	case "PESSOA":
				  		      	      carregarCombo($("#tipopessoa"), JSON.stringify(item['subgrupoitens'])) 	
				  		      	      break;
		  	  		         	case "ENDERECO":
				  		      	      carregarCombo($("#tipoendereco"), JSON.stringify(item['subgrupoitens'])) 	
				  		      	      break;	
		  	  		         	case "CONTATO":
				  		      	      carregarCombo($("#tipocontato"), JSON.stringify(item['subgrupoitens'])) 	
				  		      	      break;	
		  	  		         	case "CONTATOITEM":
				  		      	      carregarCombo($("#tipocontatoitens"), JSON.stringify(item['subgrupoitens'])) 	
				  		      	      break;					  		    	  
		  	  		       	}		  						  
		  			   });
  		      		   break;
