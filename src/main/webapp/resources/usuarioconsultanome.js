...
$('body').on('shown.bs.modal', '#modalusuarioconsultanome', function (e) {
    e.preventDefault();
...
$(document).on("click", "#botaopesquisarnome", function(e){	
	...
	var url =  "/usuario/nome/" + sessionStorage.getItem("id") +  "/" + $("#nomepesquisa").val()
	consultausuario(url, "consultarusuarionome")
...
consultausuario(url, "consultarusuariodoc")

});

  		
function consultausuario(url, operacao) {
	...
	function carregartabelapessoa(json) {
	     ...
	     for (i=0; i < json.length ; i++) {
	        ...
	        cols += json[i][2];        	
	        cols +=	'</td>';
	        cols += '<td class="col-xs-2" id="cidadeufmodal">' 
	        if (json[i][3] != null){
	        	cols += json[i][3] + "/" + json[i][4];	
	        }		                	
	        cols +=	'</td>';        
	        cols += '<td class="col-xs-3">';
	        cols += '<button  class="btn btn-success btn-xs " id="selecionarusuario"  type="submit" >Selecionar</button>';
	        cols += '</td>';
